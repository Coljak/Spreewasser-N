...
def run_producer(server = {"server": None, "port": None}, shared_id = None):
    ...
    config = {
        ...
        "server": "swn_monica",
        "sim.json": os.path.join(os.path.dirname(__file__), '_hohenfinow/sim-min.json'),
        "crop.json": os.path.join(os.path.dirname(__file__), '_hohenfinow/crop-min.json'),
        "site.json": os.path.join(os.path.dirname(__file__), '_hohenfinow/site-min.json'),
        "climate.csv": os.path.abspath(os.path.join(os.path.dirname(__file__), '_hohenfinow/climate-min.csv')),
        "debugout": "debug_out",
        "writenv": True,
        "shared_id": shared_id 
    }
    ...
    with open(config["climate.csv"]) as _:
        climate_csv = _.read()
        print("climate_csv:", climate_csv)

    env = monica_io3.create_env_json_from_json_config({
        "crop": crop_json,
        "site": site_json,
        "sim": sim_json,
        "climate": climate_csv #climate_csv
    })
    # print("env:", env)
