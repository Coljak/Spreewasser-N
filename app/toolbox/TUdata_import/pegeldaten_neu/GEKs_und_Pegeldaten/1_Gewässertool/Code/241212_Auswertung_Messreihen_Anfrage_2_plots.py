...
for datei in os.listdir(ordner_pfad):
    if datei.endswith(".csv"):
        ...
        for original, ersatz in ersetzungen.items():
            messstellenname_bereinigt = messstellenname_bereinigt.replace(original, ersatz)
    
        messstellenname_bereinigt = messstellenname_bereinigt.replace(' ', '_')  # Leerzeichen durch Unterstriche ersetzen

          # Farben für die Monate definieren
        farbschema = {
            1: 'blue',      # Januar - Winter
            2: 'lightblue',      # Februar - Winter
            3: 'cyan', # März - Frühling
            4: 'green',     # April - Frühling
            5: 'lime',      # Mai - Frühling
            6: 'yellow',    # Juni - Sommer
            7: 'orange',    # Juli - Sommer
            8: 'red',       # August - Sommer
            9: 'gold',     # September - Herbst
            10: 'brown',     # Oktober - Herbst
            11: 'gray',     # November - Herbst/Winter
            12: 'darkblue'  # Dezember - Winter
        }
        
        # Hinzufügen von Farben zu den Punkten basierend auf dem Monat
        farben = [farbschema[datum.month] for datum in monatsmittelwerte.index]
        
        # Plot erstellen
        plt.figure(figsize=(12, 6))
        plt.scatter(monatsmittelwerte.index, monatsmittelwerte.values, c=farben, edgecolor='k', label='Monatsmittelwerte')
        plt.title(f'Pegel {messstellenname} \n PKZ: {pkz}', fontsize=16)
        plt.xlabel('Monat', fontsize=14)
        plt.ylabel('Wert [cm]', fontsize=14)
        plt.grid(True, linestyle='--', alpha=0.6)
        plt.legend(fontsize=12)
        #plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%b %Y'))  # Monatsnamen anzeigen
        # Legende erstellen
        legend_elements = [Line2D([0], [0], marker='o', color='w', markerfacecolor=color, markersize=10, label=monat)
                           for monat, color in zip(
                               ["Januar", "Februar", "März", "April", "Mai", "Juni", 
                                "Juli", "August", "September", "Oktober", "November", "Dezember"], 
                               farbschema.values())]
        plt.legend(handles=legend_elements, title="Monate", ncol=4, fontsize=10, title_fontsize=12)
        ...
        dateiname = f"{pkz}_{messstellenname_bereinigt}_{startdatum_str}_bis_{enddatum_str}.png"
        ...
        ergebnisse.append({
            ...
            "MW_1990_1999_cm": jahresmittel_1990_1999,
            "Stdev [cm]": std_abw,
            "20a_Trend": trend_slope,
        ...
        print(f"Startdatum: {startdatum}")
        print(f"Enddatum: {enddatum}")
